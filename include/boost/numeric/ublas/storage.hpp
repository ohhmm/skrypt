...
#pragma warning(pop)
        ...
        unbounded_array (size_type size, const value_type &init, const ALLOC &a = ALLOC()):
            alloc_ (a), size_ (size) {
            if (size_) {
                data_ = std::allocator_traits<ALLOC>::allocate(alloc_, size_);
                for (pointer p = begin(); p != end(); ++p) {
                    std::allocator_traits<ALLOC>::construct(alloc_, p, init);
                }
            }
            else
                data_ = nullptr;
        ...
        unbounded_array (const unbounded_array &c):
            ...
            if (size_) {
                ...
                for (size_type i = 0; i < size_; ++i) {
                    std::allocator_traits<ALLOC>::construct(alloc_, data_ + i, c.data_[i]);
                }
            }
            else
                data_ = nullptr;
    ...
    private:
        ...
        void resize_internal (const size_type size, const value_type init, const bool preserve) {
            if (size != size_) {
                ...
                if (size) {
                    data_ = std::allocator_traits<allocator_type>::allocate(alloc_, size);
                    if (preserve) {
                        pointer si = p_data;
                        pointer di = data_;
                        if (size < size_) {
                            for (; di != data_ + size; ++di) {
                                std::allocator_traits<allocator_type>::construct(alloc_, di, *si);
                                ++si;
                            }
                        }
                        else {
                            ...
                            for (; di != data_ + size; ++di) {
                                std::allocator_traits<allocator_type>::construct(alloc_, di, init);
                            }
                        }
                    }
                    else {
...
#pragma warning(pop)
                            ...
                            for (pointer di = data_; di != data_ + size; ++di)
                                std::allocator_traits<allocator_type>::construct(alloc_, di, value_type());
                        }
                    }
                }
...
#pragma warning(pop)
                    ...
                    std::allocator_traits<allocator_type>::deallocate(alloc_, p_data, size_);
                }
    ...
    public:
        ...
        unbounded_array &operator = (const unbounded_array &a) {
            if (this != &a) {
                resize (a.size_);
                // Properly construct copies using allocator_traits
                for (size_type i = 0; i < a.size_; ++i) {
                    std::allocator_traits<allocator_type>::destroy(alloc_, data_ + i);
                    std::allocator_traits<allocator_type>::construct(alloc_, data_ + i, a.data_[i]);
                }
            }
            return *this;
        }
    ...
    private:
        ...
        // Removed in favor of using allocator_traits for destruction
        allocator_type alloc_;
        size_type size_;
        pointer data_;  // Pointer to fixed-size storage
    };
    ...
    public:
        ...
        BOOST_UBLAS_INLINE
        bounded_array ():
            alloc_(), size_ (0), data_(nullptr) {   // size 0 - use bounded_vector to default construct with size N
        ...
        bounded_array (size_type size):
            alloc_(), size_ (size), data_(nullptr) {
        ...
        bounded_array (size_type size, const value_type &init):
            ...
            BOOST_UBLAS_CHECK (size_ <= N, bad_size ());
            data_ = std::allocator_traits<ALLOC>::allocate(alloc_, N);
            // Construct all elements with init value
            for (size_type i = 0; i < N; ++i) {
                std::allocator_traits<ALLOC>::construct(alloc_, data_ + i, init);
            }
        }
        BOOST_UBLAS_INLINE
        ...
        ~bounded_array() {
            ...
            std::allocator_traits<ALLOC>::deallocate(alloc_, data_, N);
        }
        ...
        bounded_array &operator = (const bounded_array &a) {
            if (this != &a) {
                ...
                for (size_type i = 0; i < a.size_; ++i) {
                    std::allocator_traits<ALLOC>::destroy(alloc_, data_ + i);
                    std::allocator_traits<ALLOC>::construct(alloc_, data_ + i, a.data_[i]);
                }
            }
            return *this;
        }
    ...
    private:
        allocator_type alloc_;
        size_type size_;
        pointer data_;  // Storage allocated using allocator_traits
    };
    ...
    class array_adaptor:
        public storage_array<array_adaptor<T, ALLOC> > {

        typedef array_adaptor<T, ALLOC> self_type;
    public:
        typedef ALLOC allocator_type;
        ...
        BOOST_UBLAS_INLINE
        array_adaptor ():
            alloc_(), size_ (0), own_ (true) {
            data_ = std::allocator_traits<allocator_type>::allocate(alloc_, 0);
        }
        explicit BOOST_UBLAS_INLINE
        array_adaptor (size_type size):
            alloc_(), size_ (size), own_ (true) {
            data_ = std::allocator_traits<allocator_type>::allocate(alloc_, size);
        ...
        array_adaptor (size_type size, const value_type &init):
            ...
            data_ = std::allocator_traits<allocator_type>::allocate(alloc_, size);
            for (size_type i = 0; i < size; ++i) {
                std::allocator_traits<allocator_type>::construct(alloc_, data_ + i, init);
            }
        }
        BOOST_UBLAS_INLINE
        ...
        array_adaptor (const array_adaptor &a):
            ...
            alloc_(a.alloc_), size_ (a.size_), own_ (true) {
    ...
    private:
        ...
        void resize_internal (size_type size, value_type init, bool preserve = true) {
           if (size != size_) {
                pointer data = std::allocator_traits<allocator_type>::allocate(alloc_, size);
                if (preserve) {
                    for (size_type i = 0; i < (std::min)(size, size_); ++i) {
                        std::allocator_traits<allocator_type>::construct(alloc_, data + i, data_[i]);
                    }
                    for (size_type i = (std::min)(size, size_); i < size; ++i) {
                        std::allocator_traits<allocator_type>::construct(alloc_, data + i, init);
                    }
                }
                if (own_) {
                    for (size_type i = 0; i < size_; ++i) {
